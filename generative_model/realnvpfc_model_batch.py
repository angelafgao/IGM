...
class AffineCoupling(nn.Module):
    def __init__(self, ndim, seqfrac=4, affine=True, batch_norm=True, use_dropout=True):
        super().__init__()
        ...
        if self.use_dropout == True:
            self.dropout_param = 0.25
        else:
            self.dropout_param = 0.0
...
class Flow(nn.Module):
	def __init__(self, ndim, affine=True, seqfrac=4, batch_norm=True, use_dropout=True):
		super().__init__()
...
class RealNVP(nn.Module):
    def __init__(
        self, ndim, n_flow, affine=True, seqfrac=4, permute='random', batch_norm=True, use_dropout=True
    ):
        super().__init__()
        self.blocks = nn.ModuleList()
        self.orders = []
        for i in range(n_flow):
            self.blocks.append(Flow(ndim, affine=affine, seqfrac=seqfrac, batch_norm=batch_norm, use_dropout=use_dropout))
            if permute == 'random':
